...
### 6.4 CHANGELOG更新规则
- 每次提交代码前，必须更新 `doc/CHANGELOG.md` 文件，记录本次变更内容。
- 变更分类：
  - `✨ 新增功能`：新功能实现
  - `🐛 Bug 修复`：问题修复
  - `🔧 重构与代码质量`：代码重构、性能优化、依赖更新等
  - `📝 文档更新`：文档编写或更新
- 格式要求：按功能模块分组（前端/后端），详细描述变更内容。
- 时机：在提交代码前完成更新，确保版本日志完整性。
...
- **CHANGELOG.md**：版本变更日志，记录每次功能更新、Bug修复、重构等变更内容。**重要：每次代码提交前必须更新此文件**。
