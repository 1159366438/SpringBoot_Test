...
| check_in_time | TIMESTAMP | - | - | NOT NULL | 考勤时间（UTC） |
| check_in_type | TINYINT | - | - | NOT NULL | 考勤类型：1-上班 2-下班 3-加班 |
| check_in_status | TINYINT | - | - | NOT NULL | 考勤状态：1-正常 2-迟到 3-早退 4-旷工 |
| check_in_location | VARCHAR(100) | - | - | YES | 考勤地点（如"公司"、"家"或具体地址） |
...
| idx_user_id | user_id | 普通索引 | 加速按用户查询考勤记录 |
...
```sql
-- 插入测试考勤记录（假设admin的考勤）
INSERT INTO `attendance_record` (`user_id`, `check_in_time`, `check_in_type`, `check_in_status`, `check_in_location`) VALUES
